...
import me.xiaopan.android.happynetwork.http.interceptor.AddRequestHeaderRequestInterceptor;
import me.xiaopan.android.happynetwork.http.interceptor.GzipProcessRequestInterceptor;
import me.xiaopan.android.happynetwork.http.interceptor.GzipProcessResponseInterceptor;
...
public class HttpClientManager {
    ...
    private static SchemeRegistry getDefaultSchemeRegistry(boolean fixNoHttpResponseException, int httpPort, int httpsPort) {
        if (fixNoHttpResponseException) {
            Log.d(HappyHttpClient.class.getSimpleName(), "Beware! Using the fix is insecure, as it doesn't verify SSL certificates.");
        }

        if (httpPort < 1) {
            httpPort = 80;
            Log.d(HappyHttpClient.class.getSimpleName(), "Invalid HTTP port number specified, defaulting to 80");
        }

        if (httpsPort < 1) {
            httpsPort = 443;
            Log.d(HappyHttpClient.class.getSimpleName(), "Invalid HTTPS port number specified, defaulting to 443");
        }
